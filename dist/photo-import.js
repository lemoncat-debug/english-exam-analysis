...
function openUpload(){
 ...
 $('#photoFile').onchange=async e=>{const f=e.target.files[0];if(!f)return;if(f.size>20*1024*1024){progress.textContent='图片超过 20 MB，请缩小后再导入。';return}try{img=await createImageBitmap(f);angle=0;draw();$('#rotatePhoto').disabled=false;$('#recognizePhoto').disabled=false;progress.textContent='请确认文字朝上，并选择对应页。'}catch{progress.textContent='无法读取图片，请使用 JPG、PNG 或 WebP。'}};
 ...
 $('#recognizePhoto').onclick=async()=>{
  ...
  try{
   ...
   const result=matchPhotoWords(raw,D.pages.find(p=>p.id===page).words);
   ...
   S.passage=D.pages.find(p=>p.id===page).passage;S.page=page;S.view='photo';S.sid=null;S.word=null;S.zoom=1;render();
